...
from email_verification import configure_mail, send_verification_email, verify_verification_token
...
# Flask-Mail ve JWT konfigürasyonu
...
<body>
    ...
    <script>
        ...
        function loadAdminUsers() {
            ...
            .then(res => res.json())
            .then(users => {
                console.log('Admin kullanıcıları:', users);
            ...
            .catch(error => {
                ...
                document.getElementById('adminUserCount').textContent = 'Toplam Kullanıcı: 0';
                document.getElementById('emptyState').style.display = 'block';
                document.getElementById('adminUsersTableBody').style.display = 'none';
            });
        }
...
def register():
    try:
        ...
        user_doc = {
            ...
            'user_id': user_id,
            'is_admin': is_admin,
            'is_verified': False, # Yeni alan: E-posta doğrulaması
            'created_at': datetime.now()
        }
        ...
        if send_verification_email(mail, email, user_doc['user_id']):
            logger.info(f"✅ Yeni kullanıcı kaydoldu: {username} ({user_doc['user_id']}). Doğrulama e-postası gönderildi.")
            return jsonify({'success': True, 'message': 'Kayıt başarılı! Hesabınızı aktifleştirmek için e-posta adresinize gönderilen linke tıklayın.'})
        else:
            # E-posta gönderme başarısız olursa, kullanıcıyı silmek yerine pasif bırak
            logger.error(f"❌ Kullanıcı {username} için doğrulama e-postası gönderilemedi.")
            return jsonify({'success': False, 'message': 'Kayıt başarılı ancak doğrulama e-postası gönderilemedi. Lütfen daha sonra tekrar deneyin veya yöneticinizle iletişime geçin.'})
...
def login():
    try:
        ...
        if not user.get('is_verified', False):
            return jsonify({'success': False, 'message': 'Hesabınız doğrulanmamış. Lütfen e-posta adresinize gönderilen linke tıklayarak hesabınızı aktifleştirin.'})
...
def verify_email():
    ...
    if not token:
        return render_template_string("<h1>Hata: Doğrulama token'ı bulunamadı.</h1>")

    user_id, email = verify_verification_token(token)

    if user_id == 'expired':
        return render_template_string("<h1>Hata: Doğrulama linkinin süresi dolmuş. Lütfen tekrar kaydolmayı deneyin.</h1>")
    elif user_id == 'invalid':
        return render_template_string("<h1>Hata: Geçersiz doğrulama linki.</h1>")
    elif user_id is None:
        return render_template_string("<h1>Hata: Doğrulama sırasında bilinmeyen bir hata oluştu.</h1>")

    # Kullanıcıyı veritabanında bul
    user = users_collection.find_one({'user_id': user_id, 'email': email})

    if not user:
        return render_template_string("<h1>Hata: Kullanıcı bulunamadı veya e-posta adresi eşleşmiyor.</h1>")

    # Kullanıcı zaten doğrulanmışsa
    if user.get('is_verified', False):
        return render_template_string("<h1>Başarılı: Hesabınız zaten doğrulanmış. Giriş sayfasına yönlendiriliyorsunuz...</h1><script>setTimeout(function(){ window.location.href = '/'; }, 3000);</script>")
    ...
    return render_template_string("<h1>Başarılı: E-posta adresiniz başarıyla doğrulandı! Giriş sayfasına yönlendiriliyorsunuz...</h1><script>setTimeout(function(){ window.location.href = '/'; }, 3000);</script>")
